...
export function TestRunnerModal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-50 sm:bg-slate-100/80 sm:backdrop-blur-sm flex items-center justify-center p-0 sm:p-4 overflow-hidden select-none">
      <div className="bg-white w-full h-full max-w-7xl sm:max-h-[96vh] sm:rounded-2xl shadow-none sm:shadow-2xl overflow-hidden flex flex-col mx-auto">
        
        {/* ================= 1. CBT TOP BAR HEADER ================= */}
        <div className="bg-white text-slate-800 p-2 sm:p-3.5 flex items-center justify-between border-b border-slate-200 shrink-0">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {/* Language Selector */}
            <div className="hidden sm:flex items-center gap-1 bg-slate-50 border border-slate-200 px-2.5 py-1 rounded-lg text-xs font-semibold text-slate-600">
              ...
              >
                ...
                <option value="Hindi" className="bg-white text-slate-900">Hindi (हिंदी)</option>
              </select>
            </div>

            {/* Live Countdown Timer */}
            {!isSubmitted && (
              <div className={`flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1 sm:py-1.5 rounded-xl border font-mono text-[10px] sm:text-xs font-extrabold transition ${
                isTimeLow 
                  ? 'bg-rose-50 border-rose-200 text-rose-600 animate-pulse' 
                  : 'bg-indigo-50 border-indigo-100 text-[#1a237e]'
              }`}>
            ...
            {!isSubmitted && (
              <button
                ...
                className="p-1.5 sm:p-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 hover:text-slate-900 transition sm:hidden flex items-center gap-1 font-bold"
            ...
            <button
              onClick={onClose}
              className="p-1.5 sm:p-2 rounded-xl bg-slate-100 hover:bg-rose-100 text-slate-600 hover:text-rose-600 transition"
              title="Exit Exam"
            >
              <X className="w-4 h-4" />
            </button>
        ...
        {loading ? (
          ...
          <div className="flex-1 flex flex-col overflow-hidden bg-white relative">
                  ...
                  return (
                    <button
                      ...
                      className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-2 shrink-0 border ${
                        isActive
                          ? 'bg-slate-100 text-slate-900 border-slate-300 font-extrabold shadow-2xs'
            ...
            <div className="flex-1 flex overflow-hidden relative bg-white">
              ...
              <div className="flex-1 flex flex-col overflow-y-auto p-0 sm:p-5 space-y-2 sm:space-y-4">
                
                {currentQ ? (
                  <div className="bg-white rounded-none sm:rounded-2xl border-none sm:border border-slate-200 shadow-none sm:shadow-sm p-3 sm:p-6 space-y-3 sm:space-y-4">
                    
                    {/* Question Header Meta */}
                    <div className="flex items-center justify-between pb-2 sm:pb-3 border-b border-slate-100 gap-2 flex-wrap">
                      <div className="flex items-center gap-1.5 sm:gap-2">
                        <span className="px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-md sm:rounded-lg bg-indigo-50 text-[#1a237e] text-[10px] sm:text-xs font-black">
                          Q{currentQIdx + 1}
                        </span>
                        ...
                        {currentQ.tag && (
                          <span className="px-1.5 py-0.5 rounded bg-slate-100 text-slate-600 text-[9px] sm:text-[10px] font-mono font-bold uppercase tracking-wider border border-slate-200">
                            {currentQ.tag}
                          </span>
                        )}
                      </div>
                    ...
                    {/* Question Stem Text */}
                    <div className="pt-1">
                      <RenderMathText text={currentQ.stem} className="text-[13px] sm:text-base font-semibold text-slate-900 leading-relaxed" />
                    </div>
                      ...
                      {currentQ.opts.map((optText, optIdx) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full p-2.5 sm:p-3.5 rounded-xl border text-left transition flex items-start gap-2.5 text-[12px] sm:text-sm font-medium ${
                              isSelected
              ...
              <div className={`w-72 bg-white border-l border-slate-200 flex flex-col shrink-0 z-30 transition-transform duration-200 ${
                ...
                {/* Palette Header */}
                <div className="p-3 bg-slate-50 text-slate-900 flex items-center justify-between border-b border-slate-200">
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-full bg-indigo-100 text-[#1a237e] font-bold text-xs flex items-center justify-center">
                      U
                    </div>
                    <div>
                      <div className="text-xs font-bold">Candidate: User</div>
                      <div className="text-[10px] text-slate-500">Roll: 2026-CBT-088</div>
                    </div>
                  </div>

                  <button
                    onClick={() => setIsPaletteOpen(false)}
                    className="p-1 rounded bg-slate-200 text-slate-600 hover:text-slate-900 sm:hidden"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
            ...
            <div className="bg-white border-t border-slate-200 p-2 sm:p-3 px-2 sm:px-6 flex items-center justify-between gap-1.5 sm:gap-2 shrink-0">
              <div className="flex items-center gap-1.5 sm:gap-2">
                <button
                  onClick={handleMarkForReview}
                  className="px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-lg sm:rounded-xl bg-purple-50 hover:bg-purple-100 text-purple-900 border border-purple-200 text-[10px] sm:text-xs font-bold transition hidden sm:inline-flex"
                >
                  Mark for Review
                </button>
                ...
                <button
                  ...
                  className="px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-lg sm:rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-[10px] sm:text-xs font-bold transition"
                >
              ...
              <div className="flex items-center gap-1.5 sm:gap-2">
                <button
                  ...
                  className="px-2 py-1.5 sm:px-3.5 sm:py-2 rounded-lg sm:rounded-xl bg-white border border-slate-200 text-slate-800 hover:bg-slate-100 text-[10px] sm:text-xs font-bold disabled:opacity-40 transition flex items-center gap-0.5 sm:gap-1"
                >
                ...
                <button
                  ...
                  className="px-3 py-1.5 sm:px-5 sm:py-2 rounded-lg sm:rounded-xl bg-[#1a237e] hover:bg-indigo-900 text-white font-extrabold text-[10px] sm:text-xs uppercase tracking-wide transition active-press shadow-xs flex items-center gap-0.5 sm:gap-1"
                >
                  <span>
          ...
          <div className="flex-1 bg-slate-50 overflow-y-auto p-4 sm:p-6 space-y-6">
            ...
            {/* Header Banner */}
            <div className="bg-white text-slate-900 p-5 sm:p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <span className="text-xs font-bold text-[#1a237e] uppercase tracking-widest block">
              ...
              <button
                onClick={onClose}
                className="px-5 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-200 font-black text-xs uppercase tracking-wider transition active-press shadow-xs"
              >
                Back to Dashboard
              </button>
            </div>
